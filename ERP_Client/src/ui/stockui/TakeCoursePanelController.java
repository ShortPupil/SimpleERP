...
public class TakeCoursePanelController implements Initializable {
    ...
    private void SureAddFired(ActionEvent e) {
        ...
        try {
            ...
            if (idText.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入学号！");
                return;
            }
...
//            int cid = Integer.parseInt(idText);
            ...
            if (index < 0) {
                StockMainPageController.getInstance().showErrorMessage("请选择课程！");
                return;
            }
            ...
            if (idText.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入选课年份！");
                return;
            }
            int year = Integer.parseInt(idText);
            idText = new_score.getText();
            if (idText.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入成绩！");
                return;
            }
            ...
            if (!rs2.next()) {
                rs2.close();
                StockMainPageController.getInstance().showErrorMessage("未找到对应学生：ID=" + sid);
                return;
            }
            ...
            if (!rs3.next()) {
                rs3.close();
                StockMainPageController.getInstance().showErrorMessage("未找到对应课程：ID=" + cid);
                return;
            }
            int cy = rs3.getCancelYear();
            if(cy > 0 && year >= cy){
                StockMainPageController.getInstance().showErrorMessage("该课程已经在选课时间前取消！");
                return;
            }
            if(year - startYear + 1 < rs3.getMinGrade()){
                StockMainPageController.getInstance().showErrorMessage("该学生选课时的年级低于所选课程的最低年级要求！");
                return;
            }
            rs3.close();
            ...
            if (state == 0) {
                ...
                StockMainPageController.getInstance().showErrorMessage("校验时产生数据库错误：" + e1.getLocalizedMessage());
            }else if (state == 2) {
    ...
    public void loadAllCourses(){
        ...
        new BackgroundRunner() {
            ...
            public void finish() {
                ...
                StockMainPageController.getInstance().showMessage("已加载" + total + "个课程");
