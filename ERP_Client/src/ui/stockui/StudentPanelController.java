...
public class StudentPanelController implements Initializable {
    ...
    private void LookUpFired(ActionEvent e) {
        ...
        StockMainPageController.getInstance().showMessage("正在查询中...");
        new BackgroundRunner() {
            ...
            public void handleException(Exception e) {
                StockMainPageController.getInstance().showErrorMessage("查询数据出错：" + e.getLocalizedMessage());
                e.printStackTrace();
            }

            @Override
            public void finish() {
                StockMainPageController.getInstance().showMessage("已找到" + total + "个匹配的记录");
            }
        }.run();
    }
    ...
    private void JumpPageFired(ActionEvent e) {
        StockMainPageController.getInstance().showErrorMessage("本功能正在建设中...");
    }
    ...
    private void AddFired(ActionEvent e) {
        StockMainPageController.getInstance().showMessage("正在准备数据...");
    ...
    private void UpdateFired(ActionEvent e) {
        if (table.getSelectionModel().getSelectedItem() == null) {
            StockMainPageController.getInstance().showErrorMessage("请先选择需要修改的记录！");
            return;
        }
        StockMainPageController.getInstance().showMessage("正在准备数据...");
    ...
    private void DeleteFired(ActionEvent e) {
        if (table.getSelectionModel().getSelectedItem() == null) {
            StockMainPageController.getInstance().showErrorMessage("请先选择需要删除的记录！");
            return;
        }
        sureAddBtn.setVisible(false);
        sureUpdateBtn.setVisible(false);
    ...
    private void CalculateFired(ActionEvent e) {
        ...
        if (record == null) {
            StockMainPageController.getInstance().showErrorMessage("请先选择需要进行统计的目标！");
            return;
        }
        ...
        StockMainPageController.getInstance().showMessage("正在进行统计...");
        ...
        new BackgroundRunner() {
            ...
            public void handleException(Exception e) throws Exception {
                StockMainPageController.getInstance().showErrorMessage("加载数据出错：" + e.getLocalizedMessage());
                e.printStackTrace();
            }
            ...
            public void finish() throws Exception {
                if (total_count <= 0) {
                    StockMainPageController.getInstance().showErrorMessage("该生没有选课记录！");
                    return;
                }
                ...
                StockMainPageController.getInstance().showMessage("统计完成！");
            }
        }.run();
    }
    ...
    private void SureAddFired(ActionEvent e) {
        ...
        try {
            ...
            if (idText.length() != 10) {
                StockMainPageController.getInstance().showErrorMessage("学号长度必须是10位！");
                return;
            }
            long id = Long.parseLong(idText);
            String name = new_name.getText();
            if (name.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入姓名！");
                return;
            }
            int gender = new_gender.getSelectionModel().getSelectedIndex();//控件保证了要么是男，要么是女
            if (gender != 0 && gender != 1) {
                StockMainPageController.getInstance().showErrorMessage("请选择性别！");
                return;
            }
            ...
            if (sy.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入入学年份！");
                return;
            }
            if (sa.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入入学年龄！");
                return;
            }
            int startYear = Integer.parseInt(sy);
            int startAge = Integer.parseInt(sa);
            if (startAge < 10 || startAge > 50) {
                StockMainPageController.getInstance().showErrorMessage("入学年龄超出范围[10,50]！");
                return;
            }
            String className = (String) new_class.getSelectionModel().getSelectedItem();
            if (className == null) {
                StockMainPageController.getInstance().showErrorMessage("请选择班级！");
                return;
            }
            ...
            StockMainPageController.getInstance().showMessage("记录已成功添加！");
        } catch (Exception e1) {
            if (state == 0) {
                StockMainPageController.getInstance().showErrorMessage("数据格式不正确！" + e1.getLocalizedMessage());
            } else if (state == 1) {
                StockMainPageController.getInstance().showErrorMessage("数据库错误：" + e1.getLocalizedMessage());
            } else {
                StockMainPageController.getInstance().showErrorMessage("表格控件错误：" + e1.getLocalizedMessage());
            }
            e1.printStackTrace();
        }
    }
    ...
    private void SureUpdateFired(ActionEvent e) {
        ...
        try {
            ...
            showBottomPanel(false);
            StockMainPageController.getInstance().showMessage("记录已成功更新！");
        } catch (Exception e1) {
            if (state == 0) {
                StockMainPageController.getInstance().showErrorMessage("数据格式不正确！" + e1.getLocalizedMessage());
            } else if (state == 1) {
    ...
    private void SureDeleteFired(ActionEvent e) {
        ...
        try {
            ...
            showBottomPanel(false);
            StockMainPageController.getInstance().showMessage("记录已成功删除！");
        } catch (Exception e1) {
            if (state == 0) {
                StockMainPageController.getInstance().showErrorMessage("记录读取出错！" + e1.getLocalizedMessage());
            } else if (state == 1) {
                StockMainPageController.getInstance().showErrorMessage("数据库错误：" + e1.getLocalizedMessage());
            } else {
                StockMainPageController.getInstance().showErrorMessage("表格控件错误：" + e1.getLocalizedMessage());
    ...
    public void loadTable() {
        ...
        new BackgroundRunner() {
            ...
            @Override
            public void finish() {
                StockMainPageController.getInstance().showMessage("已加载" + total + "条记录");
            }
        }.run();
    ...
    private void loadAllClasses() {
        ...
        new BackgroundRunner() {
            ...
            @Override
            public void handleException(Exception e) throws Exception {
                StockMainPageController.getInstance().showErrorMessage("加载班级出错：" + e.getLocalizedMessage());
                e.printStackTrace();
            }
            ...
            public void finish() throws Exception {
                ...
                StockMainPageController.getInstance().showMessage("已加载" + total + "个班级");
