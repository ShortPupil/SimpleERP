...
public class CoursePanelController implements Initializable {
    ...
    private void CalculateFired(ActionEvent e) {
        ...
        new BackgroundRunner() {
            ...
            public void finish() throws Exception {
                if (total_num <= 0 ) {
                    StockMainPageController.getInstance().showErrorMessage("该课程没有选课记录！");
                    return;
                }
    ...
    private void SureAddFired(ActionEvent e) {
        ...
        try {
            ...
            if (idText.length() != 7) {
                StockMainPageController.getInstance().showErrorMessage("课程ID必须是7位！");
                return;
            }
            int id = Integer.parseInt(idText);
            String name = new_name.getText();
            if (name.length() == 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入名称！");
                return;
            }
            int index = new_tid_tname.getSelectionModel().getSelectedIndex();
            if (index < 0) {
                StockMainPageController.getInstance().showErrorMessage("请选择教师！");
                return;
            }
            ...
            if (pointStr.length() <= 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入学分！");
                return;
            }
            int point = Integer.parseInt(pointStr);
            if (point <= 0) {
                StockMainPageController.getInstance().showErrorMessage("学分必须大于0！");
                return;
            }
            String min_grade_str = new_min_grade.getText();
            if (min_grade_str.length() <= 0) {
                StockMainPageController.getInstance().showErrorMessage("请输入最低接受年级！");
                return;
            }
    ...
    public void loadAllTeachers() {
        ...
        new BackgroundRunner() {
            ...
            public void finish() {
                ...
                StockMainPageController.getInstance().showMessage("已加载" + total + "位教师");
