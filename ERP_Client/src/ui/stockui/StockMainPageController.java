 ...
 * @author E40-G8C
 */
public class StockMainPageController implements Initializable {

    private static StockMainPageController instance;

    public static StockMainPageController getInstance() {
        return instance;
    }
    @FXML
    ...
    private void studentBtnFired(ActionEvent e) {
        setPanel(StockMain.studentPanel, "打开学生管理面板");
    ...
    private void classBtnFired(ActionEvent e) {
        setPanel(StockMain.classPanel, "打开班级管理面板");
        Button btn = (Button)e.getSource();
        if(defaultBtn != null)
            defaultBtn.setDefaultButton(false);
        btn.setDefaultButton(true);
    ...
    private void courseBtnFired(ActionEvent e) {
        setPanel(StockMain.coursePanel, "打开课程管理面板");
        Button btn = (Button)e.getSource();
        if(defaultBtn != null)
            defaultBtn.setDefaultButton(false);
        btn.setDefaultButton(true);
    ...
    private void teacherBtnFired(ActionEvent e) {
        setPanel(StockMain.teacherPanel, "打开教师管理面板");
        Button btn = (Button)e.getSource();
        if(defaultBtn != null)
            defaultBtn.setDefaultButton(false);
        btn.setDefaultButton(true);
    ...
    private void takeCourseBtnFired(ActionEvent e) {
        setPanel(StockMain.takeCoursePanel, "打开选课管理面板");
        Button btn = (Button)e.getSource();
        if(defaultBtn != null)
            defaultBtn.setDefaultButton(false);
        btn.setDefaultButton(true);
...
//        StudentPanelController.getInstance().loadTable();
    ...
    public void initPanel() {
        final AnchorPane panel = StockMain.studentPanel;
        showMessage("正在初始化面板...");
        EventHandler<ActionEvent> eh = new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
