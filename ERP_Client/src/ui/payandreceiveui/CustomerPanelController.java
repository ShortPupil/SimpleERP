...
import businesslogic.customerbl.CustomerBLServiceImpl;
...
import ui.mainui.BackgroundRunner;
import vo.CustomerVO;

public class CustomerPanelController implements Initializable {

	CustomerBLService customerDataService = new CustomerBLServiceImpl();
	
    private static CustomerPanelController instance;

    public static CustomerPanelController getInstance() {
        return instance;
    }
    @FXML
    ...
    VBox bottomPanel;
    @FXML
    TableView<CustomerPO> table;
    @FXML
    Button sureAddBtn, sureUpdateBtn, sureDeleteBtn;
    @FXML
    Label deletePrompt;
    @FXML
    ChoiceBox<String> new_gender;
    @FXML
    ComboBox<String> new_class;
    @FXML
    HBox buttonPane;
    @FXML
    ...
    private void LookUpFired(ActionEvent e) {
        ...
        new BackgroundRunner() {
            ...
            public void background() throws Exception {
                ...
                ArrayList<CustomerPO> customers = customerDataService.searchCustomerByField(input);
                totalNum = customers.size();
               for(int i=0 ;  i<customers.size() ; i++){      	   
            	   CustomerPO rs = customers.get(i);                 
                   /* if (!rs2.next()) {
                        rs2.close();
                	...
                	email = rs.getEmail();
                	postcode = rs.getPostcode();
                	receivablesamount = rs.getReceivablesamount();
                	receivables = rs.getReceivables();
                	pay = rs.getPay();
                	salesman = rs.getSalesman();
                    runForeground();
                }
            }
            ...
            public void foreground() {
                table.getItems().add(new CustomerPO(id, role, level, name, phonenumber,address,email, 
                		postcode,receivablesamount, receivables, pay, salesman));
            }
            ...
            @Override
            public void finish() {
                MainPageController.getInstance().showMessage("已找到" + totalNum + "个匹配的记录");
            }
        }.run();
    }
    ...
    private void CalculateFired(ActionEvent e) {
        CustomerPO record = table.getSelectionModel().getSelectedItem();
        if (record == null) {
    ...
    private void SureAddFired(ActionEvent e) {
        ...
        try {
            ...
            String genderText = new_gender.getSelectionModel().getSelectedItem();
            ...
            String className = new_class.getSelectionModel().getSelectedItem();
            if (className == null) {
            ...
            state = 1;
            CustomerManager.addStudent(id, name, gender == 1, startYear, startAge, classID);
            state = 2;
            table.getItems().add(new CustomerTableRecord("" + id, name, genderText, startYear, startAge, className));
    ...
    private void SureUpdateFired(ActionEvent e) {
        int state = 0;//0-prepare;1-execSQL;2-addToTable
        try {
            CustomerTableRecord record = table.getSelectionModel().getSelectedItem();
            ...
            int classID = classIDList.get(new_class.getSelectionModel().getSelectedIndex());
            state = 1;
            CustomerManager.updateStudent(id, name, gender == 1, startYear, startAge, classID);
            state = 2;
            record.setId(id + "");
            record.setName(name);
    ...
    private void SureDeleteFired(ActionEvent e) {
        ...
        try {
            ...
            state = 1;
            CustomerManager.deleteStudent(Long.parseLong(id));
    ...
    public void loadTable() {
        ...
        new BackgroundRunner() {
            ...
            @Override
            public void background() throws Exception {
                StudentRecordSet rs = CustomerManager.getAllStudents();
                while (rs.next()) {
                    total++;
            ...
            @Override
            public void foreground() {
                table.getItems().add(new CustomerTableRecord(id, name, gender, sy, sa, className));
            }
    ...
    private void loadAllClasses() {
        ...
        new BackgroundRunner() {
            ...
            public void finish() throws Exception {
                ...
                CustomerTableRecord rec = table.getSelectionModel().getSelectedItem();
                if (rec != null) {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        new_class.getItems().clear();
        table.getColumns().addAll(TableColumnBuilder.create().text("ID").cellValueFactory(new PropertyValueFactory("id")).prefWidth(110).build(),
        ...
        table.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                CustomerPO record = (CustomerPO) table.getSelectionModel().getSelectedItem();
                if (record == null) {
                    return;
                }
                showBottomPanel(false);
                ...
                new_gender.getSelectionModel().select(record.getSalesman().equals("男") ? 0 : 1);
                new_start_year.setText(record.getId() + "");
                new_start_age.setText(record.getName() + "");
                new_class.getSelectionModel().select(record.getLevel());
            }
        });
    }
}
